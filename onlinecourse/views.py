...
def submit(request, course_id):
    ...
    return redirect(f'/onlinecourse/course/{str(course_id)}/submission/{str(submission.id)}/result/')
...
def show_exam_result(request, course_id, submission_id):
    ...
    lesson = choices[0].question.lesson
    
    questions = {}
    for choice in choices:
        key = str(choice.question.id)
        if (key in questions.keys()):
            questions[str(choice.question.id)] += [choice.id]
        else:
            questions[str(choice.question.id)] = [choice.id]
